#!/usr/bin/env python3
"""
Fixed Multi-GPU wav2vec2_2d training with proper DDP handling
"""
...
def run_wav2vec2_2d_fixed_ddp(session_data, output_path, num_epochs=10):
    """Main training function with fixed DDP handling"""
    ...
    if rank == 0:
        print(f"Starting Fixed Multi-GPU training on {world_size} GPUs")
    ...
    # Model configuration - FIXED to ensure all parameters are used
    use_spatial_embedding = False  # Keep disabled to avoid unused parameters
    ...
    w2v2_2d_config = Wav2Vec2_2DConfig(
        ...
        # Legacy spatial embedding parameters (disabled to avoid unused parameters)
        ...
        # Temporal parameters - ENABLED to use temporal_conv1d
        temporal_conv1d_enabled=True,
        temporal_steps=50,
        
        # Pooling parameters - ENABLED to use flatten_pool
        flattened_pool_dim=256,
        
        # Logging
        log_compression=False,
        log_temp=0.0,
        target_glu=False,  # Keep disabled to avoid unused parameters
        feature_pen=0.0,
        prob_ppl_weight=0.0,
        infonce=False,
    ...
    try:
        ...
        # CRITICAL FIX: Use find_unused_parameters=True and broadcast_buffers=False
        model = DDP(
            model, 
            device_ids=[local_rank], 
            output_device=local_rank, 
            find_unused_parameters=True,
            broadcast_buffers=False  # This helps with unused parameters
        )
    ...
    if rank == 0:
        try:
            ...
            model_path = f"{output_path}/{session_data['session_id']}/ssl_model/wav2vec2_2d_fixed_ddp.pt"
...
def main():
    """Main function to run fixed multi-GPU training"""
    parser = argparse.ArgumentParser(description='Fixed Multi-GPU wav2vec2_2d training')
    ...
    for i, session_data in enumerate(sessions_list):
        ...
        try:
            run_wav2vec2_2d_fixed_ddp(session_data, args.output_path, args.num_epochs)
        except Exception as e:
            print(f"Error in run_wav2vec2_2d_fixed_ddp: {e}")
