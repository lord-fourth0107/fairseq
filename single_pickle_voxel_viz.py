#!/usr/bin/env python3
"""
Single Pickle Voxel Visualization
=================================
Creates 1mm³ voxels with different colors for each probe from a single pickle file.
"""
...
def create_3d_voxel_plot(voxel_grid, grid_info, probe_id, probe_color, output_dir="single_pickle_viz"):
    """Create 3D plot with colored voxels for the probe."""
    
    os.makedirs(output_dir, exist_ok=True)
    print(f"Creating 3D voxel visualization for probe {probe_id}...")
    ...
    # Plot occupied voxels with probe-specific color
    ax.scatter(ap_coords, dv_coords, lr_coords, 
              c=probe_color, s=1000, alpha=0.8, edgecolors='black', linewidth=1)
    ...
    ax.set_title(f'Probe {probe_id} - Channel Locations (1mm³ Voxels)', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'probe_{probe_id}_voxels.png'), dpi=300, bbox_inches='tight')
    print(f"Saved: {os.path.join(output_dir, f'probe_{probe_id}_voxels.png')}")
    plt.close()

def create_voxel_cube_plot(voxel_grid, grid_info, probe_id, probe_color, output_dir="single_pickle_viz"):
    """Create 3D plot with actual cube representation of voxels."""
    
    print(f"Creating 3D cube visualization for probe {probe_id}...")
    ...
    for i in range(len(ap_coords)):
        ...
        for face in faces:
            ...
            ax.plot_surface(
                ...
                color=probe_color, alpha=0.7, edgecolor='black', linewidth=0.5
            )
    ...
    ax.set_ylabel('Dorsal-Ventral (μm)', fontsize=14)
    ax.set_zlabel('Left-Right (μm)', fontsize=14)
    ax.set_title(f'Probe {probe_id} - Channel Locations (1mm³ Voxel Cubes)', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'probe_{probe_id}_cubes.png'), dpi=300, bbox_inches='tight')
    print(f"Saved: {os.path.join(output_dir, f'probe_{probe_id}_cubes.png')}")
    plt.close()
...
def main():
    parser = argparse.ArgumentParser(description='Single Pickle Voxel Visualization')
    ...
    parser.add_argument('--output-dir', default='single_pickle_viz', 
                       help='Output directory for visualizations (default: single_pickle_viz)')
    parser.add_argument('--voxel-size', type=float, default=1.0,
                       help='Voxel size in mm (default: 1.0)')
    
    args = parser.parse_args()
    
    print("Single Pickle Voxel Visualization")
    print("="*40)
    ...
    try:
        ...
        create_3d_voxel_plot(voxel_grid, grid_info, probe_id, probe_color, args.output_dir)
        create_voxel_cube_plot(voxel_grid, grid_info, probe_id, probe_color, args.output_dir)
