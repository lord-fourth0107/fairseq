#!/usr/bin/env python3
"""
PyTorch-only Multi-GPU wav2vec2_2d training - no fairseq dependencies
"""
...
class SimpleWav2Vec2_2D(nn.Module):
    """Simple wav2vec2_2d model with Scaled RoPE - PyTorch only"""
    
    def __init__(self, input_height, input_width, embed_dim=32, num_layers=2, num_heads=2):
        super().__init__()
        ...
        conv_h = input_height // 4  # 2 stride-2 layers
        conv_w = input_width // 4
        conv_out_dim = 16 * conv_h * conv_w
        
        # Projection to embed_dim
        self.projection = nn.Linear(conv_out_dim, embed_dim)
        
        # Scaled RoPE
        self.rope = SimpleScaledRoPE(embed_dim, max_seq_len=1024)
        ...
        # Simple quantizer
        self.quantizer = nn.Linear(embed_dim, 32)  # 32 codebook entries
    ...
    def forward(self, source, padding_mask=None):
        ...
        features = self.conv_layers(source)  # [B, 16, H/4, W/4]
        
        # Flatten and project
        features = features.view(batch_size, -1)  # [B, 16*H*W]
        ...
        # Quantization
        quantized = self.quantizer(encoded)  # [B, 1, 32]
...
def run_wav2vec2_2d_pytorch(session_data, output_path, num_epochs=3):
    ...
    if rank == 0:
        print(f"Starting PyTorch-only Multi-GPU training on {world_size} GPUs")
        print(f"Session: {session_data['session_id']}")
    
    # Load data - ultra minimal
    try:
        if isinstance(session_data['data'], np.ndarray):
            data = session_data['data']
        else:
            # For large lists, sample a tiny subset
            raw_data = session_data['data']
            if len(raw_data) > 50:  # If more than 50 elements
                print(f"[Rank {rank}] Large dataset detected: {len(raw_data)} elements. Sampling 50 for training...")
                # Sample every nth element to get ~50 samples
        ...
        # Create dataset with tiny chunks
        chunk_size = min(5, data.shape[0])  # Process in chunks of 5
        dataset = []
    ...
    if rank == 0:
        try:
            ...
            model_path = f"{output_path}/{session_data['session_id']}/ssl_model/wav2vec2_2d_pytorch.pt"
...
def main():
    """Main function to run PyTorch-only multi-GPU training"""
    parser = argparse.ArgumentParser(description='PyTorch-only Multi-GPU wav2vec2_2d training')
    ...
    for i, session_data in enumerate(sessions_list):
        ...
        try:
            run_wav2vec2_2d_pytorch(session_data, args.output_path, args.num_epochs)
        except Exception as e:
            print(f"Error in run_wav2vec2_2d_pytorch: {e}")
